...
func ShortUrl(url string) []string {
	...
	sMD5EncryptResult := hex.EncodeToString(md5Hash.Sum(nil))	//32位16进制数

	resUrl := make([]string, 4)


	for i := 0; i < 4; i++ {
		//与03xFFFFFFF（001111...）进行位与运算（清零高2位）保留后30位
		sTempSubString := sMD5EncryptResult[i*8 : i*8+8]

		//转换为整数
		lHexLong, _ := strconv.ParseInt(sTempSubString, 16, 64)

		lHexLong = 0x3FFFFFFF & lHexLong
		outChars := ""


		//每个短码6位
		for j := 0; j < 6; j++ {
			index := 0x0000003D & lHexLong		//保留低六位  3D=00111101=61  0~61
			outChars += chars[index]
			lHexLong = lHexLong >> 5		//处理下一个
		}
		resUrl[i] = outChars	
	}
	return resUrl
...
func (s *UrlServiceImpl) AddUrl(ctx context.Context, req *url.UrlRequest) (resp *url.UrlResponse, err error) {
	...
	//存入数据库
	u:= model.Url{LongUrl:longUrl, ShortCode: shortUrl,Visited: 0,CreatedUser: middleware.UserName}
